...
def LogicAdd(mg1,mg2,x1,y1,x2,y2):
 ...
 if n==1:  
    rows,cols,channels=mg2.shape
    roi=mg1[y1-190:rows+y1-190,x1-70:cols+x1-70]
    rows2,cols2,channels=roi.shape
    if rows2!=cols2:
        return
    img2gray=cv.cvtColor(mg2, cv.COLOR_BGR2GRAY)
    ...
    img2_fg=cv.bitwise_and(mg2,mg2,mask=mask)
    dst=cv.add(img1_bg,img2_fg)
    mg1[y1-190:rows+y1-190,x1-70:cols+x1-70]=dst
    
    return mg1
 if n==2:  
    rows,cols,channels=mg2.shape
    roi=mg1[y1-230:rows+y1-230,x1-70:cols+x1-70]
    rows2,cols2,channels=roi.shape
    print(rows2,"  ",cols2)
    if rows2!=cols2:
        return
    ...
    img2_fg=cv.bitwise_and(mg2,mg2,mask=mask)
    dst=cv.add(img1_bg,img2_fg)
    mg1[y1-230:rows+y1-230,x1-70:cols+x1-70]=dst
    return mg1
